...
export class TeneoWebsocketClient {
	...
	private setupWebSocketListeners(): Promise<void> {
		return new Promise((resolve, reject) => {
			if (!this.webSocket) {
				reject(new Error(chalk.red('❌ WebSocket instance not initialized')));
				return;
			}
			...
			this.webSocket.on('error', (error) => {
				this.state = WebSocketConnectionState.DISCONNECTED;
				cleanup();
				logMessage('error', chalk.red('❌ WebSocket error:'), error.message);
				reject(error);
			});
	...
	public async ping(): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			this.webSocket!.send(JSON.stringify(pingData), (err) => {
				if (err) {
					reject(
						new Error(chalk.red(`❌ Failed to send ping: ${err.message}`))
					);
					return;
				}
				logMessage(
					'success',
					chalk.blue(
						`Ping sent${this.proxyConfig ? ` using ${this.getProxyUrl()}` : ''}`
					)
				);
				resolve();
			});
